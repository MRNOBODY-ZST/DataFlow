    ...
    <div class="flex flex-1 flex-col justify-center px-4 py-12 sm:px-6 lg:flex-none lg:px-20 xl:px-24">
      <div class="mx-auto w-full max-w-sm lg:w-96">
        <div>
          ...
          <h2 class="mt-8 text-2xl/9 font-bold tracking-tight text-gray-900 dark:text-white">{{ t('auth.signInTitle') }}</h2>
          <p class="mt-2 text-sm/6 text-gray-500 dark:text-gray-400">
            {{ t('auth.noAccount') }}
            {{ ' ' }}
            <router-link to="/register" class="font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300">{{ t('auth.createOne') }}</router-link>
          </p>
        </div>
