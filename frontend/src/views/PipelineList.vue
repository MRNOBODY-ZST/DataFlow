  ...
  <AppLayout :title="t('pipeline.title')">
    <div class="py-6 px-4 sm:px-6 lg:px-8">
      <div class="sm:flex sm:items-center sm:justify-between">
        ...
        <button type="button" class="mt-4 rounded-md bg-sky-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-sky-500 sm:mt-0 dark:bg-sky-500 dark:shadow-none dark:hover:bg-sky-400" @click="openCreate">
      ...
      <div class="mt-6 overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-gray-200 dark:bg-gray-800/75 dark:ring-white/10">
        ...
        <div v-else-if="pipelineStore.pipelines.length === 0" class="px-6 py-16 text-center">
          <div class="mx-auto max-w-sm rounded-lg border border-dashed border-gray-300 px-6 py-10 dark:border-white/20">
            ...
            <div class="mt-6">
              <button type="button" class="rounded-md bg-sky-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-sky-500 dark:bg-sky-500 dark:hover:bg-sky-400" @click="openCreate">{{ t('pipeline.newPipeline') }}</button>
            </div>
          </div>
        </div>
                ...
                <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  ...
                  <div class="mt-4 space-y-4">
                    <div>
                      ...
                      <input v-model="newName" required class="mt-2 block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-sky-600 sm:text-sm/6 dark:bg-white/5 dark:text-white dark:outline-white/10 dark:focus:outline-sky-500" />
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-gray-900 dark:text-gray-100">{{ t('pipeline.descLabel') }}</label>
                      <input v-model="newDesc" class="mt-2 block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-sky-600 sm:text-sm/6 dark:bg-white/5 dark:text-white dark:outline-white/10 dark:focus:outline-sky-500" />
                    </div>
                  </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 dark:bg-gray-800/50">
                  <button type="submit" class="inline-flex w-full justify-center rounded-md bg-sky-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-sky-500 sm:ml-3 sm:w-auto dark:bg-sky-500 dark:hover:bg-sky-400">{{ t('common.create') }}</button>
